...
const cameraFallbackImage =
  'https://lh3.googleusercontent.com/aida-public/AB6AXuCPNTQ7LPRPRg8B095T_AlgZDwilQvpLt8C-T8-7uBlvJl9OkueEFiaFpuXAinxaMP8QTXQ1ADcEN10YzxUU3xZZe4wystY5CaVogont9j49zeECVvj7I5YENUGIt40KTjb68TqxVoSm4Y0ywM4xzzHMAuL2QZe2i59QCzyCxB1aFJ7l721aPhfFLLovRVbjziir15JMadleY1TAoUYMfqol2HB2Bzv91oIpPDkKkqd4F24ZE_NEBN6bx4CO_wvbx_rxR6qG2Ryk8JI';

const getTrafficLevel = (risk) => {
  if (risk >= 70) return 'heavy';
  if (risk >= 40) return 'slow';
  return 'smooth';
};

const getStatusColor = (flowLabel) => {
  const lower = String(flowLabel || '').toLowerCase();
  if (lower.includes('heavy')) return 'text-red-400';
  if (lower.includes('slow')) return 'text-yellow-400';
  if (lower.includes('free')) return 'text-green-400';
  return 'text-slate-300';
};

const splitLocation = (locationName) => {
  const [name, city] = String(locationName || 'Unknown Intersection').split(',').map((v) => v.trim());
  return {
    name: name || 'Unknown Intersection',
...
export const LocationProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const syncFromBackend = async () => {
      try {
        const [intersections, cameras, recommendations] = await Promise.all([
          fetchJson('/intersections'),
          fetchJson('/cameras'),
          fetchJson('/recommendations/top'),
        ]);

        const recMap = new Map(
          recommendations.map((rec) => [rec.targetIntersectionId, rec])
        );

        const camByIntersection = new Map();
        cameras.forEach((cam) => {
          const parts = String(cam.cameraId || '').split('-');
          const intersectionId = parts.length >= 3 ? `${parts[1]}-${parts[2]}` : '';
          if (!camByIntersection.has(intersectionId)) camByIntersection.set(intersectionId, []);
          camByIntersection.get(intersectionId).push(cam);
        });

        const liveRows = await Promise.all(
          intersections.map((it) =>
            fetchJson(`/intersections/${it.intersectionId}/live`).catch(() => null)
          )
        );

        const mapped = intersections.map((it, idx) => {
          const live = liveRows[idx]?.live || liveRows[idx] || null;
          const rec = recMap.get(it.intersectionId);
          const loc = splitLocation(it.locationName);
          const risk = Number(live?.densityPercent || 0);
          const cams = camByIntersection.get(it.intersectionId) || [];

          return {
            id: it.intersectionId,
            name: loc.name,
            city: loc.city,
            lat: -7.2575,
            lng: 112.7521,
            trafficLevel: getTrafficLevel(risk),
            congestionRisk: Math.round(risk),
            avgSpeed: Math.round(Number(live?.avgSpeedKmh || 0)),
            queueLength: Math.round(Number(live?.queueLengthVehicles || 0)),
            waitTime: Math.round(Number(live?.waitTimeMinutes || 0)),
            flowRate: Math.round(Number(live?.flowRateCarsPerMin || 0)),
            vehicles: Math.round(Number(live?.currentVolume || 0)),
            weather: {
              temp: Math.round(Number(live?.weatherTempC || 0)),
              desc: live?.weatherCondition || 'Unknown',
              humidity: 0,
            },
            peakTime: '17:00',
            networkVelocity: Math.round(Number(live?.avgSpeedKmh || 0)),
            criticalHotspots: risk >= 80 ? 1 : 0,
            cameras: (cams.length ? cams : [{ label: 'Cam 1 - Default', flowLabel: 'Unknown' }]).map(
              (cam, camIdx) => ({
                id: camIdx + 1,
                name: cam.label,
                status: cam.flowLabel || 'Unknown',
                statusColor: getStatusColor(cam.flowLabel),
                image: cameraFallbackImage,
              })
            ),
            recommendation: {
              alert: rec?.alertTitle || 'No active recommendation',
              desc: rec?.alertDescription || 'Sistem belum mengeluarkan rekomendasi baru.',
              currentGreen: `${rec?.currentGreenSeconds ?? it.currentSignalPlan.greenSeconds}s`,
              recommended: `${rec?.recommendedGreenSeconds ?? it.currentSignalPlan.greenSeconds}s`,
              delta: `${rec?.deltaSeconds > 0 ? '+' : ''}${rec?.deltaSeconds ?? 0}s`,
            },
          };
        });
    ...
    syncFromBackend();
    const timer = setInterval(syncFromBackend, POLL_MS);
  ...
  useEffect(() => {
    ...
    setSelectedLocation((prev) => {
      ...
      const stillExists = locations.find((loc) => loc.id === prev.id);
  ...
  const value = useMemo(
    ...
    [locations, selectedLocation, isBackendConnected]
  );
