...
// -------------------------------------------------------
const PredictionSidebar = ({ onSelectLocation, locations }) => {
  const safeLocations = locations || [];
  const totalHotspots = safeLocations.reduce((a, l) => a + (l.criticalHotspots || 0), 0);
  const avgVelocity = safeLocations.length
    ? Math.round(safeLocations.reduce((a, l) => a + (l.networkVelocity || 0), 0) / safeLocations.length)
    : 0;
  const sorted = [...safeLocations].sort((a, b) => b.congestionRisk - a.congestionRisk);
...
const LiveMapPage = () => {
  ...
  return (
    <div className="relative w-full" style={{ height: 'calc(100vh - 64px)' }}>
      ...
      <PredictionSidebar onSelectLocation={handleMarkerClick} locations={locations} />
